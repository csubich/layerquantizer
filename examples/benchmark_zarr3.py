...
async def run_benchmark(args):
    ...
    # Each config is (label, list_of_zarr3_codecs)
    configs = []
    if args.mode == "simple":
        configs = [
            ("Blosc(zstd, clevel=5)", [BloscCodec(typesize=4, cname='zstd', clevel=5, shuffle='shuffle')]),
            ("LQ (16-bit)", [LayerQuantizerCodec(nbits=16), BloscCodec(typesize=4, cname='zstd', clevel=5, shuffle='shuffle')]),
        ]
    elif args.mode == "sweep":
        # Test Blosc(zstd) alone with various clevels
        for clevel in [1, 5, 9]:
            configs.append((f"Blosc(zstd, clevel={clevel})", 
                            [BloscCodec(typesize=4, cname='zstd', clevel=clevel, shuffle='shuffle')]))
        ...
        for nbits in [8, 12, 16]:
            for clevel in [1, 5, 9]:
                configs.append((f"LQ (nbits={nbits}, clevel={clevel})", 
                                [LayerQuantizerCodec(nbits=nbits), 
                                 BloscCodec(typesize=4, cname='zstd', clevel=clevel, shuffle='shuffle')]))
    ...
    for var_name in VARIABLES_3D:
        ...
        # Clear encoding to avoid carrying over Zarr 2 settings
        for v in dv.variables:
            dv[v].encoding = {}
        raw_bytes = dv.nbytes
        
        for label, codecs in configs:
            print(f"  Testing {label}...")
            # Zarr 3 MemoryStore
            store = zarr.storage.MemoryStore()
            
            # For Zarr 3 in Xarray, we use serializer and compressors
            is_lq = any(isinstance(c, LayerQuantizerCodec) for c in codecs)
            if is_lq:
                lq_codec = next(c for c in codecs if isinstance(c, LayerQuantizerCodec))
                other_codecs = [c for c in codecs if not isinstance(c, LayerQuantizerCodec)]
                encoding = {var_name: {
                    "serializer": lq_codec,
                    "compressors": other_codecs,
                }}
            else:
                encoding = {var_name: {
                    "compressors": codecs,
            ...
            # Note: zarr_format=3 for Zarr 3; consolidated=False to avoid Zarr 3 warnings
            dv.to_zarr(store, compute=True, encoding=encoding, zarr_format=3, consolidated=False)
            write_time = (datetime.datetime.now() - start_write).total_seconds()
            
            # Read benchmark
            start_read = datetime.datetime.now()
            reload = xr.open_zarr(store, zarr_format=3, consolidated=False).compute()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Benchmark LayerQuantizer with Zarr 3")
